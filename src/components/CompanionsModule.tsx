...
export const CompanionsModule: React.FC<CompanionsModuleProps> = ({
  ...
  const handleCreateSubmit = (e: React.FormEvent) => {
    ...
    const created: CompanionTrip = {
      ...
      organizer: {
        ...
        experienceYears: 4,
        completedTrips: 8,
        fstrRank: currentUser?.experienceLevel || 'Организатор сплавов',
        phone: currentUser?.phone || '+7 (999) 000-00-00',
        telegram: newTelegramLink ? `@${newTelegramLink.replace('@', '').replace('https://t.me/', '')}` : '@splav_crew'
      },
      description: newDesc || 'Совместный сплав по красивейшей северной реке. Ищем надежных попутчиков!',
      requiredExperience: (newExperience as any) || 'Средний (2-4 сплава)',
      ...
      chatMessages: [
        {
          ...
          role: 'organizer',
          text: `Экспедиция создана! Приветствуем будущих участников сплава по реке ${newRiver}. Задавайте вопросы в чате.`,
  ...
  const handleDeleteTrip = (tripId: string) => {
    askConfirmation({
      ...
      message: 'Вы действительно хотите удалить этот сплав? Это действие необратимо.',
      confirmText: 'Да, удалить сплав',
  ...
  const handleLeaveCrew = (trip: CompanionTrip) => {
    ...
    askConfirmation({
      title: 'Покинуть экипаж сплава?',
      message: `Вы действительно хотите выйти из состава экипажа сплава "${trip.title}"? Ваше место освободится для других участников.`,
  ...
  const handleRemoveParticipantFromCrew = (trip: CompanionTrip, person: { name: string; userId?: string; phone?: string }) => {
    askConfirmation({
      ...
      message: `Участник ${person.name} будет удален из списка экипажа, освободив место на сплаве.`,
  ...
  const handleSendChatMessage = async (trip: CompanionTrip) => {
    if (!canAccessTripChat(trip)) {
      alert('Чат доступен только участникам сплава, принятым в основной экипаж.');
      return;
    }
  ...
  const handleDownloadTripGpx = (trip: CompanionTrip) => {
    if (!trip.gpxTrack || !trip.gpxTrack.coordinates || trip.gpxTrack.coordinates.length === 0) {
      alert('У этого сплава еще нет прикрепленного GPX трека.');
      return;
    }
      ...
      <div className="bg-white border border-[#E5E0D8] rounded-[28px] p-5 sm:p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#6B665F] mt-0.5">
            Присоединяйтесь к проверенным экипажам, загружайте GPX треки рек и находите надежную команду для водных сплавов
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>Организовать сплав</span>
          </button>
        </div>
      </div>

      {/* EXPEDITION STATUS TABS (Active / All / Archive) */}
      <div className="bg-[#FAF7F2] border border-[#E5E0D8] rounded-2xl p-2 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="grid grid-cols-1 sm:flex sm:flex-wrap items-center gap-1.5 w-full sm:w-auto">
          <button
            onClick={() => setViewTab('active')}
            className={`w-full sm:w-auto px-4 py-2.5 sm:py-2 rounded-xl text-xs font-bold transition-all flex items-center justify-center sm:justify-start gap-1.5 cursor-pointer ${
              viewTab === 'active'
          ...
          >
            ...
            <span>Все сплавы ({trips.length})</span>
          </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-[#8B7E6D] mt-0.5">
                Здесь хранятся все прошедшие сплавы, история экипажей, отзывы и прикрепленные GPX треки.
              </p>
            </div>
          </div>
        </div>
        ...
        {filteredTrips.length === 0 && (
          <div className="col-span-full py-12 text-center bg-white border border-[#E5E0D8] rounded-[24px] p-8 space-y-4">
            ...
            <div className="space-y-1 max-w-md mx-auto">
              <h3 className="font-bold text-sm text-[#1A1F1A]">Сплавов с выбранными параметрами не найдено</h3>
              <p className="text-xs text-[#8B7E6D]">
                В данной категории пока нет активных наборов. Вы можете переключиться на вкладку «Все сплавы», сбросить фильтры или создать новый сплав.
              </p>
            </div>
            <div className="flex items-center justify-center gap-2.5 flex-wrap pt-1">
              {viewTab !== 'all' && (
                ...
                >
                  <Users className="w-3.5 h-3.5" />
                  <span>Показать все сплавы ({trips.length})</span>
                </button>
              )}
              ...
              <div className="flex items-center gap-3 bg-[#F9F7F4] p-2.5 rounded-2xl border border-[#EEEBE6]">
                ...
                <div className="overflow-hidden">
                  ...
                  <div className="text-[10px] text-[#8B7E6D] truncate">
                    {trip.organizer.fstrRank} • {trip.organizer.completedTrips} сплавов
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-3 border-t border-[#E5E0D8]">
                ...
                {isActualOrganizer(trip) ? (
                  <button
                    ...
                    title="Вы капитан сплава: управление заявками"
                  >
                  ...
                  <button
                    ...
                    title="Вы в основном экипаже: открыть чат сплава"
                  >
              ...
              <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
                ...
                {isTripExpiredOrArchived(selectedTrip) && (
                  <div className="p-3 bg-[#FCFAF7] border border-[#E0D7CB] rounded-2xl flex items-center gap-2 text-xs text-[#7A5B3E]">
                    ...
                    <span>Сплав находится в архиве (завершен по дате старта). Данные участников и чат сохранены.</span>
                  </div>
                )}
                    ...
                    {isUserParticipant(selectedTrip) ? (
                      <div className="p-3.5 bg-[#E8F1E7] rounded-2xl border border-[#CDE0CC] flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                        <div className="flex items-center gap-2 text-[#2D5A27] font-bold">
                          ...
                          <span>Вы состоите в основном экипаже этого сплава!</span>
                        </div>
                        ...
                        <button
                          ...
                          onClick={() => {
                            askConfirmation({
                              ...
                              message: 'Вы уверены, что хотите отозвать свою заявку на участие в этом сплаве?',
                ...
                {selectedTrip.gpxTrack ? (
                  ...
                  <div className="py-10 text-center bg-[#F9F7F4] rounded-2xl border border-[#EEEBE6] space-y-3">
                    ...
                    <p className="text-xs text-[#8B7E6D] max-w-md mx-auto">
                      Организатор сплава может загрузить GPX файл через кнопку «Редактировать», чтобы на карте отображалась нитка русла реки.
                    </p>
                    {isTripOrganizer(selectedTrip) && (
                      <button
                        onClick={() => setEditingTripInModal({ ...selectedTrip })}
            ...
            {modalTab === 'chat' && (
              canAccessTripChat(selectedTrip) ? (
                <div className="space-y-3 max-h-[64vh] flex flex-col justify-between">
                  ...
                  <div className="space-y-2.5 overflow-y-auto max-h-[38vh] pr-1 scrollbar-thin">
                    {liveChatMessages.length === 0 && (
                      <div className="py-10 text-center text-xs text-[#8B7E6D] space-y-2 bg-[#F9F7F4] rounded-2xl border border-dashed border-[#DDD7CE]">
                        ...
                        <div className="font-bold text-[#1A1F1A]">Сообщений в сплаве пока нет</div>
                        <p className="text-[11px] text-[#8B7E6D]">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 py-1 text-sm border-t border-[#EEEBE6]/80">
                  ...
                  <div className="pt-2 border-t border-[#E5E0D8] flex items-center gap-2">
                    <input
                      type="text"
                      placeholder={currentUser ? "Написать онлайн в чат экипажа (Enter для отправки)..." : "Напишите сообщение в чат сплава..."}
                ...
                <div className="py-8 px-4 sm:px-8 text-center bg-[#F9F7F4] rounded-[24px] border border-[#EEEBE6] space-y-4 max-w-lg mx-auto my-2">
                  ...
                  <div className="space-y-1.5">
                    <h4 className="text-base font-black text-[#1A1F1A]">
                      Закрытый чат участников сплава
                    </h4>
                    <p className="text-xs text-[#6B665F] leading-relaxed">
                      Чат сплава, координация экипажа и внутренние договоренности доступны только участникам, чьи заявки одобрены капитаном ({selectedTrip.organizer.name}).
                    </p>
                  </div>
                  ...
                  {!currentUser ? (
                    <div className="pt-2 space-y-3">
                      <div className="p-3 bg-white rounded-xl border border-[#E5E0D8] text-xs text-[#4A443E] flex items-center justify-center gap-2">
                        ...
                        <span>Войдите в профиль, чтобы подать заявку на участие в сплаве.</span>
                      </div>
                      <button
                        onClick={onOpenAuth}
                    ...
                    <div className="pt-2 space-y-3">
                      <div className="p-3.5 bg-[#FEF3C7] rounded-xl border border-[#FDE68A] text-xs text-[#92400E] space-y-1.5 text-left">
                        ...
                        <p className="text-[11px] text-[#78350F] leading-relaxed">
                          Капитан рассматривает вашу заявку. Доступ к закрытому чату и экипажу откроется автоматически сразу после одобрения заявки.
                        </p>
                      </div>
                    ...
                    <div className="pt-2 space-y-3">
                      <div className="p-3.5 bg-[#FDE8E8] rounded-xl border border-[#F8B4B4] text-xs text-[#991B1B] space-y-1.5 text-left">
                        ...
                        <p className="text-[11px] text-[#7F1D1D] leading-relaxed">
                          К сожалению, организатор отклонил заявку на участие в данном сплаве. Внутренний чат экипажа недоступен.
                        </p>
                      </div>
                    </div>
                  ) : (
                    <div className="pt-2 space-y-3">
                      ...
                      {Math.max(0, selectedTrip.totalSeats - selectedTrip.bookedSeats) > 0 ? (
                        ...
                        >
                          ...
                          <span>Подать заявку на участие в сплаве</span>
                        </button>
                      ) : (
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-[2800] bg-black/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="bg-white border border-[#E5E0D8] rounded-[28px] max-w-2xl w-full max-h-[92vh] overflow-y-auto p-5 sm:p-6 space-y-4 shadow-2xl my-6 text-[#2D332D]">
            ...
            <div className="flex items-start justify-between border-b border-[#E5E0D8] pb-3">
              <div>
                <h3 className="text-base font-black text-[#1A1F1A]">Создание сплава / экспедиции</h3>
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-3.5 text-xs">
              <div>
                <label className="block text-[#4A443E] font-bold mb-1">Название сплава *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-[#4A443E] font-bold mb-1">Описание сплава и план заброски</label>
                <textarea
                  rows={3}
                  value={newDesc}
                  onChange={(e) => setNewDesc(e.target.value)}
                  placeholder="Расскажите о реке, заброске, графике сплава, требованиях к участникам..."
                  className="w-full bg-[#F9F7F4] border border-[#E5E0D8] rounded-xl p-2.5 text-[#2D332D] outline-none focus:border-[#2D5A27]"
                />
              </div>
          ...
          <div className="bg-white border border-[#E5E0D8] rounded-[28px] max-w-2xl w-full max-h-[92vh] overflow-y-auto p-5 sm:p-6 space-y-4 shadow-2xl my-auto text-[#2D332D]">
            <div className="flex items-center justify-between pb-3 border-b border-[#E5E0D8]">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-black text-[#1A1F1A]">Редактирование параметров сплава</h3>
            ...
            <form onSubmit={handleSaveEditedTripInModal} className="space-y-3.5 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="completed">Сплав завершен (Архив)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingTripInModal({ ...editingTripInModal, requiredExperience: e.target.value as any })}
                    placeholder="Например: Средний (2-4 сплава)"
                    className="w-full bg-[#F9F7F4] border border-[#E5E0D8] rounded-xl p-2.5 text-[#2D332D] outline-none focus:border-[#2D5A27]"
                  />
                </div>
              ...
              <div>
                <label className="block text-[#4A443E] font-bold mb-1">Описание сплава и план</label>
                <textarea
                  rows={3}
