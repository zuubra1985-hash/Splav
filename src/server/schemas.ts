...
export const registerUserSchema = z.object({
  ...
  password: z.string().min(6, 'Пароль должен содержать не менее 6 символов').max(128, 'Пароль не должен превышать 128 символов'),
...
export const tripOrganizerSchema = z.object({
  ...
  avatar: z.string().max(2000000).default(''),
...
export const tripApplicationSchema = z.object({
  ...
  applicantAvatar: z.string().max(2000000).optional(),
...
export const tripChatMessageSchema = z.object({
  ...
  authorAvatar: z.string().max(2000000).optional().default(''),
...
export const articleSchema = z.object({
  ...
  coverImage: z.string().max(2000000).default(''),
  content: z.string().max(50000).default(''),
  tags: z.array(z.string().max(100)).max(50).default([]),
  authorId: z.string().max(100).optional(),
  authorAvatar: z.string().max(2000000).optional(),
